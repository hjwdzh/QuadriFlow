...
inline Vector3d TravelField(Vector3d p, Vector3d& pt, double& len, int& f, VectorXi& E2E, MatrixXd& V, MatrixXi& F, MatrixXd& NF,
	MatrixXd& QF, MatrixXd& QV, MatrixXd& NV,
	std::vector<MatrixXd>& triangle_space, double* tx = 0, double* ty = 0, Vector3d* dir_unfold = 0) {
	Vector3d N = NF.col(f);
